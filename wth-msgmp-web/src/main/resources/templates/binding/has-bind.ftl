    ...
    window.onload = function () {
      ...
      document.body.addEventListener('click', function (e) {
        ...
        switch (id) {
          // 修改绑定手机号
          case 'editPhone':
            window.location.href = '${springUrl}wx/phoneBinding?id='+document.getElementById('_id').value+'&openId='+document.getElementById('openId').value
            break;
          // 取消绑定手机号
          case 'cancelBind':
      ...
      function toggleConfirm (flag,type) {
        ...
        if (!flag && type == "ok"){
          $.ajax({
            type: 'get',
            url: '${springUrl}wx/deleted?id='+document.getElementById('_id').value,
            cache: false,
            dataType: "json",
            success: function(result) {
              if(result.code == 0){
                window.location.href = '${springUrl}wx/phoneBinding?openId='+document.getElementById('openId').value
              }
